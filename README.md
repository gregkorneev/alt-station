...
Добро пожаловать в проект ALT‑Station! Это сборник скриптов, системных единиц и инструкций для превращения ноутбука или рабочей станции под управлением ALT Linux в полноценный «мини‑сервер» для собственных нужд. Проект включает:

Бот Telegram для мониторинга батареи, температуры и состояния вентилятора — отправляет уведомления о разряде и позволяет выполнять безопасные команды или (для администратора) открыть интерактивную оболочку. Подробности и установка описаны в документе Bot.md
.

Веб‑доступ к файлам — варианты как организовать совместный доступ к вашим каталогам: от локального SMB‑шаринга через Samba (подключаемого из приложения Файлы на iPad) до защищённого веб‑серверирования с помощью Tailscale Serve. Инструкции по настройке приведены в file.md
.

Удалённый VS Code — инструкция, как развернуть веб‑версию редактора code‑server на ALT Linux и подключаться к нему из браузера (iPad или другого компьютера), либо использовать расширение VS Code Remote SSH. Подробный гайд находится в vs_code.md
.

Структура репозитория
alt-station/
├── battery_bot.py           # основной скрипт Telegram‑бота
├── batterybot.service       # пример unit‑файла systemd для бота
├── docs/                    # папка с дополнительными руководствами
│   ├── Bot.md               # подробная документация по боту
│   ├── file.md              # настройка доступа к файлам
│   └── vs_code.md           # установка и настройка удалённого VS Code
└── README.md                # этот файл (корневой путеводитель)

Как пользоваться

Ознакомьтесь с Bot.md
, чтобы запустить бота мониторинга. В документе перечислены все команды, требования и инструкция по установке как службы systemd.

Если вам нужен доступ к файлам станции через веб или приложение Файлы на iPad, прочитайте file.md
. Там описывается, как настроить Samba для локального доступа и как опубликовать каталог через Tailscale Serve.

Для работы с кодом удалённо, настройте code‑server согласно vs_code.md
 и подключайтесь к нему из браузера. Это позволит писать код или управлять системой с любого устройства.

Кто этот проект для

Проект рассчитан на владельцев ноутбуков/станций под ALT Linux, которые хотят:

получать уведомления о разряде батареи и состоянии системы на телефон;

быстро включать доступ к файловой системе для загрузки документов или медиа на iPad/ноутбук;

работать с кодом и терминалом через браузер, не находясь за столом.

Все инструкции написаны на русском языке и рассчитаны на пользователей, ранее не сталкивавшихся с подобными задачами. Следуя пошаговым гайдам, вы сможете настроить мониторинг, обмен файлами и удалённый доступ на «чистой» системе ALT Workstation.

Обратная связь

Если у вас есть предложения по улучшению инструкций или вы нашли ошибку, создайте issue или сделайте pull request в этот репозиторий. Благодарим за участие!
